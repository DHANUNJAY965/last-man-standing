...
contract LastManStanding is ReentrancyGuard {
    ...
    function _handleBonusDepositor(uint256 depositNumber, address depositor, uint256 potAtTime) internal {
        ...
        // Make VRF request to external randomness contract
        try randomnessSender.requestRandomness() returns (uint256 requestId) {
            ...
            _handleRandomnessFulfilled(randomnessRequestCounter, keccak256(abi.encode(block.timestamp, block.difficulty, depositor)));
        }
    }

    // This function will be called by the randomness oracle
    ...
    // This function will be called by the blocklock oracle when data can be revealed
    ...
    // For testing: manual trigger of randomness (remove in production)
    function testTriggerRandomness(uint256 counter, bytes32 randomValue) external onlyOwner {
        _handleRandomnessFulfilled(counter, randomValue);
    }

    // For testing: manual trigger of blocklock reveal (remove in production)
    function testRevealBonus(uint256 requestId, uint256 percentage) external onlyOwner {
        _revealBonusPercentage(requestId, percentage);
    }
